...
def get_interpolations(args, model, device, images, images_per_row=20):
    ...
    with torch.no_grad():
        ...
        if args.model == 'VAE':
            mu, logvar = model.encode(images.view(-1, 4096))
            embeddings = model.reparameterize(mu, logvar).cpu()
        elif args.model == 'AE':
            embeddings = model.encode(images.view(-1, 4096))
        ...
        for i in range(0, images_per_row+1, 1):
            ...
            line = torch.cat((images[i].view(-1, 4096), interp_dec, images[i+1].view(-1,4096)))
            interps.append(line)
        # Complete the loop and append the first image again
        interp = interpolate(embeddings[i+1], embeddings[0], images_per_row-4)
        interp = interp.to(device)
        interp_dec = model.decode(interp)
        line = torch.cat((images[i+1].view(-1, 4096), interp_dec, images[0].view(-1,4096)))
        interps.append(line)
